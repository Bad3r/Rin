...
export function createBaseApp(env: Env): Router {
  ...
  app.use(async (context: Context) => {
    ...
    const cache =
      context.store?.cache ??
      (await container.get('cache', async () => {
    ...
    const serverConfig =
      context.store?.serverConfig ??
      (await container.get('serverConfig', async () => {
        const { CacheImpl } = await import('../utils/cache')
        const lazyDb = await container.get('db', async () => {
          const { drizzle } = await import('drizzle-orm/d1')
          const schema = await import('../db/schema')
    ...
    const clientConfig =
      context.store?.clientConfig ??
      (await container.get('clientConfig', async () => {
        const { CacheImpl } = await import('../utils/cache')
        const lazyDb = await container.get('db', async () => {
          const { drizzle } = await import('drizzle-orm/d1')
          const schema = await import('../db/schema')

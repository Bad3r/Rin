  ...
  renderDesktop({ children, profile, siteConfig, isAtTop }) {
    return (
      <div className='hidden w-full lg:block'>
        <div
          className={`flex items-center justify-between gap-8 px-4 py-3 ${isAtTop ? 'bg-transparent backdrop-blur-none' : 'bg-white/20 backdrop-blur-xl dark:bg-white/[0.03]'}`}
        >
          <BrandLink
          ...
          <div className='flex min-w-0 flex-1 items-center justify-end'>
            <div className='flex min-w-0 flex-row items-center justify-end text-sm t-primary'>
              <NavBar
                menu={false}
                itemClassName='px-0 py-1 pr-3 md:p-0 md:pr-3 text-sm font-medium text-neutral-700 dark:text-neutral-200'
              />
            </div>

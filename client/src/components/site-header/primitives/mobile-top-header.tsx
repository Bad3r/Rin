...
export function MobileTopHeader({
  ...
  return (
    <div
      className={`flex w-full items-center justify-between gap-3 px-4 py-2 lg:hidden ${isAtTop ? 'bg-transparent backdrop-blur-none' : 'bg-white/20 backdrop-blur-xl dark:bg-white/[0.03]'}`}
    >
        ...
        {showInlineNav ? (
          <div className='hidden min-w-0 flex-1 items-center sm:flex'>
            <div className='flex min-w-max items-center overflow-x-auto'>
              <NavBar
                menu={false}
                itemClassName='px-0 py-1 pr-3 md:p-0 md:pr-3 text-sm text-neutral-600 dark:text-neutral-300'
              />
            </div>
          </div>
        ) : null}

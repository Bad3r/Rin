  ...
  const AlertUI = () => (
    ...
    >
      <div className='flex flex-col items-start p-4 bg-w space-y-4 w-full min-w-56 sm:min-w-96'>
        <h1 className='text-2xl font-bold t-primary'>{t('alert')}</h1>
        <p className='text-base t-primary'>{alert?.message}</p>
...
export function useConfirm() {
  ...
  const ConfirmUI = () => (
    ...
    >
      <div className='flex flex-col items-start p-4 bg-w space-y-4 w-full min-w-56 sm:min-w-96'>
        <h1 className='text-2xl font-bold t-primary'>{confirm?.title}</h1>
        <p className='text-base t-primary'>{confirm?.message}</p>
        <div className='w-full flex flex-row items-center justify-center space-x-2 mt-4'>
          <ButtonWithLoading
            loading={loading}
            onClick={async () => {

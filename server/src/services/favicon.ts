  ...
  router.post(
    ...
    async (ctx: Context) => {
      ...
      try {
        ...
        const originFaviconKey = path_join(env.S3_FOLDER || '', `originFavicon${FAVICON_ALLOWED_TYPES[file.type]}`)

        await putObject(s3, env, originFaviconKey, file)
        ...
        await putObject(s3, env, faviconKey, new Uint8Array(arrayBuffer))

      ...
      <div className='toc-content dark:text-neutral-300'>
        <ReactMarkdown
          ...
          components={{
            ...
            a({ children, ...props }) {
              return (
                <a className='text-[#0686c8] dark:text-[#2590f1] hover:underline' {...props}>
                  {children}
                </a>
              )
            },
            ...
            th: ({ node, ...props }) => <th className='px-4 py-2 border bg-gray-600' {...props} />,
            ...
            section({ children, ...props }) {
              const sectionClassName = Object.hasOwn(props, 'data-footnotes')
                ? `${props.className || ''} mt-8`.trim()
                : props.className
              const modifiedChildren = React.Children.map(children, child => {
                if (isValidElement<SectionChildProps>(child) && child.props.node?.tagName === 'ol') {

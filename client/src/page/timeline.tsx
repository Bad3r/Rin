        ...
        <main className='w-full flex flex-col justify-center items-center mb-8 ani-show'>
          <div className='wauto text-start text-black dark:text-white py-4 text-4xl font-bold'>
            ...
            <div className='flex flex-row justify-between'>
              <p className='text-sm mt-4 text-neutral-500 font-normal'>{t('article.total$count', { count: length })}</p>
            </div>
          </div>
          {feeds &&
            Object.keys(feeds)
              ...
              .map(year => (
                <div key={year} className='wauto flex flex-col justify-center items-start'>
                  <h1 className='flex flex-row items-center space-x-2'>
                    <span className='text-2xl font-bold t-primary '>{t('year$year', { year: year })}</span>

...
async function devCommand(args: string[]) {
  ...
  if (values.client) {
    ...
    const proc = Bun.spawn(['bun', '--filter', './client', 'dev'], {
      stdout: 'inherit',
      stderr: 'inherit',
    })
  ...
  if (values.server) {
    ...
    const proc = Bun.spawn(['bun', 'wrangler', 'dev', '--port', String(PORT), '--test-scheduled'], {
      stdout: 'inherit',
      stderr: 'inherit',
  ...
  const buildProc = Bun.spawn(['bun', '--filter', './client', 'build'], {
    stdout: 'inherit',
    stderr: 'inherit',
  })

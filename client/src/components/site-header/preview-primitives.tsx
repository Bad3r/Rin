...
export function PreviewCanvas({
  ...
  return (
    <div className={`space-y-3 ${className}`.trim()} style={style}>
      {children}
    </div>
  )
}
    ...
    <div
      className={
        vertical
          ...
          : `flex items-center ${compact ? 'gap-3' : 'gap-4'} ${center ? 'justify-center' : ''}`
      }
    >
...
export function PreviewActions({
  ...
  return (
    <div className={stacked ? 'flex flex-col items-start gap-1.5' : 'flex items-center justify-end gap-1.5'}>
      <div
        className={`h-6 w-6 rounded-full ${minimal ? 'bg-black/[0.06] dark:bg-white/[0.08]' : 'border border-black/10 dark:border-white/10'}`}
      />
      <div
        className={`h-6 w-6 rounded-full ${minimal ? 'bg-black/[0.06] dark:bg-white/[0.08]' : 'border border-black/10 dark:border-white/10'}`}
      />
...
function PreviewAvatar({
  ...
  return avatar ? (
    <img
      src={avatar}
      alt=''
      className={`${compact ? 'h-7 w-7 rounded-xl' : 'h-8 w-8 rounded-full'} shrink-0 object-cover`}
    />
  ) : (
    <div
      className={`${compact ? 'h-7 w-7 rounded-xl' : 'h-8 w-8 rounded-full'} shrink-0`}
...
function PreviewName({ name, compact = false }: { name: string; compact?: boolean }) {
  ...
  return (
    <p
      className={`truncate font-semibold text-neutral-900 dark:text-neutral-100 ${compact ? 'max-w-16 text-[10px]' : 'max-w-24 text-[10px]'}`}
    >
      {displayName}
    </p>
  )
